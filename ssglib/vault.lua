...
local function find_local_links(doc)
  ...
  doc:walk({
    Link = function(link)
      if utils.is_local_path(link.target) then
        table.insert(links, link.target)
      end
      return nil
    end,
    Image = function(img)
      if utils.is_local_path(img.src) then
...
local function read_note_properties(path)
  ...
  if meta.tags and type(meta.tags) == "table" then
    props.tags = meta.tags:filter(stringify)
  end
...
--- Get the filename (without directory path or extension) from the path.
--- @return string filename The filename without extension
function File:filename()
  assert(getmetatable(self) == File)
  return self.path:match("([^/]+)%.%w+$") or self.path:match("([^/]+)$") or self.path
end
...
local function read_metadata_cache(path)
  ...
  if not ok2 then
    utils.warn("error reading cache %s: %s", path, cache)
    return {}
  end
  if type(cache) ~= "table" or cache.schema ~= cache_schema or type(cache.files) ~= "table" then
    utils.warn("error reading cache %s: bad file format", path)
    return {}
  end
  local cache_files = {}
...
local function write_metadata_cache(path, files)
  ...
  if not file then
    utils.warn("Failed to write cache: %s", err)
    return
  end
...
function Vault:_scan(cache_path)
  ...
  utils.walk_tree(self.path, function(file_path)
    local path = utils.posix_path(pandoc.path.make_relative(file_path, self.path))
    local mtime = utils.file_mtime(file_path)
  ...
  utils.info("Vault: total=%d, updated=%d", total, updated)
end
...
--- Query vault files.
--- @param filter fun(f: File): boolean Return true to include f in result.
--- @param less fun(a: File, b:File) boolean Return true if a sorts before b.
--- @return table files Query result
function Vault:query(filter, less)
  local result = {}
