...
--- representations into properly formatted HTML/XML tags with attributes and content.
---
...
local function element_function(name, opts)
  ...
  return function(elem)
    ...
    if tag_type == "void" then
      -- Void tags (e.g., <br>, <img>) do not have closing tags or content
      table.insert(result, close)
    elseif tag_type == "xml" and not elem[1] then
      -- Self-closing XML tags if no content provided
      table.insert(result, "/>")
    else
      -- Standard tags with content
...
-- Configuration for standard HTML5 tags specifying void elements and line breaks
local html_options = {
  area = { type = "void" },
