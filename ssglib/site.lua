...
--- Pepare write to file corresponding to the specified URL.
...
function Site:prepare(url)
  ...
  local path = pandoc.path.join { self.root, utils.system_path(url:sub(2)) }
...
function Site:write_file(url, path, mtime)
  local target = self:prepare(url)
  if utils.file_mtime(target) > (mtime or utils.file_mtime(path)) then
    return
  end
  self.updated = self.updated + 1
...
function Site:write_dir(url, root)
  assert(url:find("/$"))
  utils.walk_tree(root, function(path)
    local u = url .. utils.posix_path(pandoc.path.make_relative(path, root))
...
function Site:cleanup()
  assert(getmetatable(self) == Site)
  local deleted = 0
  utils.walk_tree(self.root, function(path)
    if not self.files[path] then
      utils.info("DELETE /%s", utils.posix_path(pandoc.path.make_relative(path, self.root)))
      deleted = deleted + 1
      pandoc.system.remove(path)
    end
  end)
  utils.info("Site:  total=%d, updated=%d, deleted=%d", self.total, self.updated, deleted)
end
